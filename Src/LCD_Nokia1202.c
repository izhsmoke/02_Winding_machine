...
void LCD_Init(void) 
{
  ...
  LCD_GPIO_InitStructure.GPIO_Pin = GPIO_Pin_5 | GPIO_Pin_6 | GPIO_Pin_7 | GPIO_Pin_8 | GPIO_Pin_9;
...
{
	LCD_FillRect(Graph_X0,Graph_Ymax,Graph_Xmax,Graph_Yh,0);
	uint8_t i;
	uint16_t max = mass[0];

	/*
	//для теста заполняем массив данными АЦП
	for(i=0;i<Graph_Xmax;i++)
	{
		mass[i] = (ADC1->JDR1)-1400;
	}
	*/
	
	//поск максемального значения в массиве
	for(i=1;i<Graph_Xmax;i++)
	{
		if (mass[i] >  max)
		{
			max = mass[i];
...
//нормируем элементы массива под размер экрана	
	...
	for (i=0;i<Graph_Xmax;i++)
	{
		LCD_DrawLine(i,Graph_Y0,i,Graph_Y0-mass[i],1);
	}
}	
